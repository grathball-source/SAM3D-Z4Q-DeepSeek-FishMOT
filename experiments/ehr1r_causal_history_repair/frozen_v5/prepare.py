...
from contract import assert_packet_segments, reason, source_item, dumps
...
def clean_run(rows, native, start, stop, end_at):
    ...
    for f in range(start, stop+1):
        valid = reason(source_item(rows, f, native)) is None
        if valid:
            current.append(f)
        elif current:
            runs.append(current)
...
def condition(base,arm):
    ...
    if arm in ('E','H-2D'):
        ...
        for row in p['INTERACTION_OBSERVATIONS']:
            for o in row['anonymous_observations']:
                o.pop('depth_median_pipeline_mm',None);o.pop('depth_valid_fraction',None)
    else:
        for part in ('PRE_HISTORY','POST_HISTORY_TO_Q'):
            for s in p[part].values():
    ...
    if arm!='E':
        ...
        if arm not in ('H-2D',):columns+=['depth_median_pipeline_mm','depth_valid_fraction']
        table=[]
        for frame in p.pop('INTERACTION_OBSERVATIONS'):
            for o in frame['anonymous_observations']:
                ...
                if arm!='H-2D':row += [o['depth_median_pipeline_mm'],o['depth_valid_fraction']]
                table.append(row)
