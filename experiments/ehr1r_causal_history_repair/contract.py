...
def version_signature(row, native, observation):
    """Read identity-version metadata where the source actually provides it."""
    if observation is None:
        return ()
    ...
    return tuple((k,observation.get(k,native_row.get(k))) for k in keys
                 if k in observation or k in native_row)
...
def validate_segment(segment, rows, native, anchor=None):
    ...
    for a,b in zip(frames,frames[1:]):
        if a not in rows or b not in rows:
            continue
        delta=rows[b]['time']-rows[a]['time']
        if delta<=0 or delta>.2:
            failures.append(f'TIME_DISCONTINUITY_F{a}_F{b}')
        sa=version_signature(rows[a],native,source_item(rows,a,native))
        sb=version_signature(rows[b],native,source_item(rows,b,native))
        if sa!=sb:failures.append(f'IDENTITY_VERSION_CHANGE_F{a}_F{b}')
