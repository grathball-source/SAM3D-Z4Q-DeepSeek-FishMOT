...
def parse(content, request):
    ...
    for r in rows:
        ...
        for cite in evidence:
            if not isinstance(cite, dict) or not isinstance(cite.get('observation'), str) or not cite['observation'].strip():
                return dict(schema_valid=False, reason='BAD_CITATION', rows=[])
            pair = (cite.get('image_id'), cite.get('observation_token'))
            if not all(isinstance(x, str) for x in pair):
                return dict(schema_valid=False, reason='BAD_CITATION_TOKEN', rows=[])
            if pair[0] == core['source']['image_id'] and pair[1] in sources:
                ...
                return dict(schema_valid=False, reason='UNSENT_CITATION', rows=[])
        expected = {(core['source']['image_id'], r['source_token']),
                    (core['target']['image_id'], target)} if status == 'MATCH' else set()
        citation_valid = status == 'UNRESOLVED' or expected <= set(citations)
...
def run(root=ROOT):
        ...
        for arm in ('FIRST', 'REPEAT'):
            ...
            for token, truth_row in true.items():
                ...
                item = dict(attempt_id=attempt, pair=pair, arm=arm, source_token=token,
                            raw_target=target, decision=decision, truth_target=truth_row['target'],
    ...
    signal=(scoreable_hops>=8 and scoreable_total>=16 and
            ...
            sum(agreement_rows)/len(agreement_rows)>=.90 and
    ...
    write(root/'public/RESPONSE_MANIFEST.json', dict(status='SEALED_AND_POSTHOC_PARSED',
    ...
    jsonl(root/'public/TOKEN_MATCH_RESULTS.jsonl', tokens)
    write(root/'public/PAIR_RESULTS.json', pairs)
    jsonl(root/'public/BASELINE_RESULTS.jsonl', baseline_rows)
    write(root/'public/CHAIN_COMPOSITION.json', chains)
    write(root/'public/FAILURE_CASES.json', [x for x in tokens if x['correctness']=='WRONG' or
