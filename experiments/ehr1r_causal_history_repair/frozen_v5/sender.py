"""Single-use EHR-1R sender; mount only its audited sender tree at /work."""
...
SYSTEM = ('Compare the two complete physical identity hypotheses from the supplied causal event packet. '
          'A/B are newly frozen pre-risk source fragments; X/Y are current q-local fragments. '
          ...
          'The compact event table includes every anonymous observation, and its columns define the measurements. '
...
def body(request, uploaded):
    ...
    assert core['request_id'] == 'EHR1R-' + request['case']
...
def reserves():
    ...
    assert gate['old_packet_rejected'] is True and gate['depth_parity_pass'] is True
    assert len(gate['old_packet_rejection_sha256'])==64 and len(gate['depth_audit_sha256'])==64
    values={x['attempt_id']:x['reserve_usd'] for x in gate['request_budget']}
    values['S001']=gate['smoke_reserve_usd']
    assert len(values)==26 and gate['total_reserve_usd']<=gate['cap_usd']
    return values
...
def budget(plan):
    ...
    reserved=reserves()
    remaining=sum(value for attempt,value in reserved.items() if attempt not in starts)
...
def invoke(attempt, payload, key, plan, smoke=False):
    ...
    try:
        ...
        charge = ((prompt * .30 + completion * 1.20) / 1e6
                  if type(prompt) is int and type(completion) is int else reserve)
        ...
        assert charge <= reserve, 'budget reserve exceeded; stop'
        return public
    except Exception as e:
        if not (ROOT / 'public/responses' / (attempt + '.json')).exists():
            append(ROOT / 'public/CALL_LEDGER.jsonl', dict(phase='END', at=now(), attempt_id=attempt,
                   ...
                   charged_upper_usd=reserve, latency_seconds=time.monotonic()-began))
