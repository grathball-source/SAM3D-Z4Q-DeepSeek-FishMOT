...
def validate_event_depth_table(table, anonymous_rows):
    required={'depth_median_pipeline_mm','depth_valid_fraction','depth_sensor_available',
    ...
    by_id={o['fact_id']:o for frame in anonymous_rows for o in frame['anonymous_observations']}
    for values in table['rows']:
        fact=by_id[values[0]]
        assert all(values[table['columns'].index(k)]==fact[k] for k in required)
...
def no_depth(packet):
    ...
    if t:
        keep=[i for i,c in enumerate(t['columns']) if not c.startswith('depth_')]
        t['columns']=[t['columns'][i] for i in keep]
        t['rows']=[[x[i] for i in keep] for x in t['rows']]
    v['condition']='H-2D'
    return v
...
def verify(run):
    ...
    for packet in episodes:
        ...
        for arm in ('H-2D','H-D','H-D-REPEAT','H-D-PERMUTE'):
            ...
            if arm!='H-2D':
                validate_event_depth_table(table,packet['INTERACTION_OBSERVATIONS'])
